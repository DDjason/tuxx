...
import sys;
sys.path.append("/root/Test/cifar10")
import cifar10
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    # Get images and labels for CIFAR-10.  获取图片和标签
    eval_data = FLAGS.eval_data == 'test'
    images, labels = cifar10.inputs(eval_data=eval_data)
    filename = '/root/Test/leaf_test_image/070100068.jpg'
    imagecontent = tf.read_file(filename)
    image = tf.image.decode_jpeg(imagecontent, channels=3)
    image = tf.cast(image, tf.float32)

    image=tf.reshape(image, [32, 32, 3])

    image1 = tf.image.resize_image_with_crop_or_pad(image, 24, 24)
